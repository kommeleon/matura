...
func _3d_to_2d(points_3d: Array) -> Array:
	...
	for point in points_3d:
		if point.z < 0.1:
			flat_points.append(Vector2(-10000, -10000))
		else:
...
func _points_from_cam(points_3d: Array, camera_pos: Vector3) -> Array:
	var distance_points = []
	var basis = Basis().rotated(Vector3(0, 1, 0), camera_rot.y)
	basis = basis.rotated(basis.x, camera_rot.x)
	var cam_inv = basis.inverse()
	for point in points_3d:
...
func _input(event):
	if event is InputEventMouseMotion:
		...
		camera_rot.x = clamp(camera_rot.x, -1.57, 1.57)


func _process(delta):
	var basis = Basis()
	basis = basis.rotated(Vector3(0, 1, 0), camera_rot.y) # Yaw
	basis = basis.rotated(basis.x, camera_rot.x) # Pitch
	var forward = -basis.z 
	var side = basis.x
...
func _draw():
	...
	for p in points_to_draw:
		screen_points.append((p * scale_amount) + screen_center)

	for edge in edges:
		var p1 = screen_points[edge[0]]
		var p2 = screen_points[edge[1]]
